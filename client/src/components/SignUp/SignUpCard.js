...
const useStyles = makeStyles({
  ...
  alertNewlyRegistered: {
    marginTop: "1.25em",
  },
});

const SignUpCard = ({ auth, authActions, history }) => {
  const [userToRegister, setUserToRegister] = useState({ email: "", firstName: "", lastName: "", password: "" });
  ...
  return (
    <div>
      <Card className={classes.card} elevation={0} style={{ boxShadow: isMobileWidth ? "" : "rgb(0 0 0 / 10%) 0 0 10px" }}>
        <CardContent>
          <h1 className={classes.cardTitle}>Sign up for your account</h1>
          <form className={classes.form}>
            <TextField
              ...
              type="email"
              variant="outlined"
              onChange={(e) => setUserToRegister({ ...userToRegister, email: e.target.value })}
            ...
            <TextField
              ...
              type="text"
              variant="outlined"
              onChange={(e) => setUserToRegister({ ...userToRegister, firstName: e.target.value })}
            ...
            <TextField
              ...
              type="text"
              variant="outlined"
              onChange={(e) => setUserToRegister({ ...userToRegister, lastName: e.target.value })}
            ...
            <TextField
              ...
              onChange={(e) => setUserToRegister({ ...userToRegister, password: e.target.value })}
            ...
            <Button
              ...
              onClick={async () => {
                const registerResult = await authActions.createAccount(userToRegister);
                if (registerResult.valid) history.push("/login");
                else {
                  setErrModal({ display: true, msg: registerResult.error });
                }
              }}
              variant="contained"
              color="secondary"
